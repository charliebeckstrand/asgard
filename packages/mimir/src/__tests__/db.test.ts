...
describe('createDb', () => {
	...
	describe('many', () => {
		it('returns all rows', async () => {
			...
			const result = await db.many<{ id: number; name: string }>(sql`SELECT * FROM users`)
		...
		it('returns empty array when no rows', async () => {
			...
			const result = await db.many(sql`SELECT * FROM users`)
	...
	describe('exec', () => {
		it('returns row count', async () => {
			...
			const result = await db.exec(sql`DELETE FROM users WHERE active = ${false}`)
		...
		it('returns 0 when rowCount is null', async () => {
			...
			const result = await db.exec(sql`DELETE FROM users WHERE id = ${999}`)
	...
	describe('val', () => {
		it('returns the first column of the first row', async () => {
			...
			const result = await db.val<number>(sql`SELECT COUNT(*)::int FROM users`)
		...
		it('throws NoRowsError when no rows', async () => {
			...
			await expect(db.val(sql`SELECT COUNT(*) FROM users`)).rejects.toThrow(NoRowsError)
		})
	})
	...
	describe('tx', () => {
		...
		it('releases client even when rollback fails', async () => {
			...
			await expect(
				db.tx(async (tx) => {
					await tx.many(sql`SELECT * FROM bad_table`)
				}),
			).rejects.toThrow()
